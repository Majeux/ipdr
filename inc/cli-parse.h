...
namespace my::cli
{
  ...
  namespace algo
  {
    ...
    struct IPDR
    {
      Model_var model;
      pdr::Tactic type;
      IPDR(const Model_var& m) : model(m) {}
    };

    struct Bounded
    {
      Pebbling model;
      Bounded(const Model_var& m)
      {
        if (std::holds_alternative<Peterson>(m))
          throw std::invalid_argument(
              "Bounded model check only accepts pebbling.");
        model = std::get<Pebbling>(m);
      }
    };
