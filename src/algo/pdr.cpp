...
namespace pdr
{
	...
	bool PDR::block(std::priority_queue<MIN_ORDERING(Obligation)> obligations, unsigned level)
	{
		...
		while (obligations.size() > 0)
		{
			...
			expr_vector neg_cube = z3ext::negate(state->cube);
			if (frames[n]->added(neg_cube))
			{
				obligations.pop();
				stats.blocked_ignored++;
				SPDLOG_LOGGER_TRACE(log, "{}| obligations is already blocked at {}. ignored", TAB, n);
				continue;
			}

			expr state_clause = z3::mk_or(neg_cube);

			if ( frames[n]->SAT(state_clause, model.literals.p(state->cube)) )
			{	//predecessor found
				shared_ptr<State> pred = std::make_shared<State>(expr_vector(ctx), state);
