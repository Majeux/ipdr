...
namespace pdr 
{
	...
	int PDR::highest_inductive_frame(const z3::expr_vector& cube, int min, int max)
	{
		...
		SPDLOG_LOGGER_TRACE(logger.spd_logger, "{}| highest inductive frame is {}", logger.tab(), highest);
		return highest;
	}

	int PDR::highest_inductive_frame(const z3::expr_vector& cube, int min, int max, z3::expr_vector& core)
	{
		...
		if (result >= 0 && result >= min) //if unsat result occurs
		{
			...
			core = frames.solver(result)->unsat_core(
					[this](const z3::expr& e) { return model.literals.literal_is_p(e); }, //select next
					[this](const z3::expr& e) { return model.literals(e); }); //transform to current
		...
		SPDLOG_LOGGER_TRACE(logger.spd_logger, "{}| used substituted cube for core: {} -> {}", 
	...
	{
		std::vector<z3::expr> cube = z3ext::convert(state); //use std::vector for sorting and intersection
	...
	bool PDR::down(vector<z3::expr>& state, int level)
	{
		...
		while (true)
		{
			...
			vector<z3::expr> cti_intersect = frames.solver(level)->witness_vector(is_current_in_state);
